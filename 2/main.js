class CustomHexColorPicker {
  ...
  render() {
    ...
    if (this.showPalette) {
      this.presetColorsEle = this.paletteEle.querySelector(
        'hexcp-palette preset-colors'
      );
    } else {
      this.paletteEle.style.display = 'none';
    }

    this.customColorBoxEle = this.hexCpViewEle.querySelector(
      'hexcp-palette custom-colors .customColorBox'
    );
    ...
    this.spectrumDraggerEle = this.spectrumEle.querySelector(
      'hexcp-main .spectrum .spectrumDragger'
    );
    ...
    this.spectrumDraggerMaxX =
      this.spectrumWidth - this.spectrumDraggerEle.clientWidth / 2;
    this.spectrumDraggerMaxY =
      this.spectrumHeight - this.spectrumDraggerEle.clientHeight / 2;
    ...
    this.stripDraggerEle = this.stripEle.querySelector(
      'hexcp-main .colorStrip .colorStripDragger'
    );
    this.stripDraggerMinX = -this.stripDraggerEle.clientWidth / 2;
    this.stripDraggerMaxX =
      this.stripWidth - this.stripDraggerEle.clientWidth / 2;
  ...
  renderCustomColors() {
    var customColorsList =
      JSON.parse(localStorage.getItem(this.CUSTOM_COLOR_STORAGE_KEYNAME)) || [];
    var customColorsEleList = Array.from(
      this.customColorBoxEle.querySelectorAll('.customColor')
    );
    var customColorEle;
    var customColor;
    ...
    this.addCustomColorEle =
      this.customColorBoxEle.querySelector('.addCustomColor');
  ...
  customColorAdd() {
    ...
    var customColorEleList =
      this.customColorBoxEle.querySelectorAll('.customColor.empty');
    var customColorEle =
      customColorEleList[0 /*customColorEleList.length - 1*/];
    if (customColorEle) {
      ...
      this.customColorBoxEle
        .querySelector('.customColor')
        .before(customColorEleClone);
      customColorEleClone.onclick = this.customColorSelect.bind(this);
      customColorsList.unshift(this.currColor);
      localStorage.setItem(
        this.CUSTOM_COLOR_STORAGE_KEYNAME,
        JSON.stringify(customColorsList)
      );
    } else {
  ...
  setCurrCustomColorToFront() {
    var oldCustomColorEle = this.customColorBoxEle.querySelector(
      `.customColor[${this.CUSTOM_COLOR_ATTRNAME}="${this.currColor}"]`
    );
    var currCustomColorEle = oldCustomColorEle.cloneNode();
    this.customColorBoxEle
      .querySelector('.customColor')
      .before(currCustomColorEle);
    currCustomColorEle.onclick = this.customColorSelect.bind(this);
    oldCustomColorEle.remove();
    var customColorsList = JSON.parse(
      localStorage.getItem(this.CUSTOM_COLOR_STORAGE_KEYNAME)
    );
  ...
  spectrumEleMousedown(event) {
    ...
    this.spectrumDraggerPosX =
      event.clientX - rect.left + this.spectrumDraggerMinX;
    this.spectrumDraggerPosY =
      event.clientY - rect.top + this.spectrumDraggerMinY;
  ...
  spectrumChange() {
    this.currentSaturation = parseFloat(
      (this.spectrumDraggerPosX - this.spectrumDraggerMinX) / this.spectrumWidth
    );
  ...
  stripChange() {
    this.currentHue = parseFloat(
      (this.stripDraggerPosX - this.stripDraggerMinX) / this.stripWidth
    );
  ...
  hexChange(setTransition = true) {
    ...
    this.setSpectrumColor(flatColor);
    this.spectrumDraggerPosX =
      this.currentSaturation * this.spectrumWidth + this.spectrumDraggerMinX;
    this.spectrumDraggerPosY =
      this.spectrumHeight -
      this.currentValue * this.spectrumHeight +
      this.spectrumDraggerMinY;
    this.spectrumDraggerEle.style.cssText = `${
      setTransition ? 'transition:0.2s;' : ''
    }left:${this.spectrumDraggerPosX}px;top:${this.spectrumDraggerPosY}px;`;

    this.stripDraggerPosX =
      this.currentHue * this.stripWidth + this.stripDraggerMinX;
    this.stripDraggerEle.style.cssText = `${
      setTransition ? 'transition:0.2s;' : ''
    }left:${this.stripDraggerPosX}px;`;
    this.removeActiveClass();
  }
  ...
  hexToRgb(hex) {
    var hexMap = (hex = hex.replace('#', '')).match(
  ...
  unitToRatio(colorUnit, maxValue) {
    var isPercentage =
      typeof colorUnit === 'string' && colorUnit.indexOf('%') != -1;
  ...
  getPrevColor() {
    var prevColor =
      typeof $ == 'function'
        ? $(this.colorInputEle).data(this.INPUT_COLOR_DATAATTRNAME)
        : this.colorInputEle.getAttribute(this.INPUT_COLOR_ATTRNAME).trim();
  ...
  hidePicker(event) {
    if (
      !event ||
      (event &&
        event.relatedTarget !== this.hexCodeEle &&
        event.relatedTarget !== this.selectColorEle)
    ) {
  ...
  selectColor() {
    ...
    typeof $ == 'function' &&
      $(this.colorInputEle).data(this.INPUT_COLOR_DATAATTRNAME, this.currColor);
